...
export default async function EditAdminProfilePage() {
  ...
  if (!userId) {
    return (
      <div className="p-8 text-center text-slate-500">
        Unauthorized. Please sign in to edit profile.
      </div>
    );
  }
  ...
  if (user.publicKey && user.encryptedPrivateKey) {
    try {
      ...
      console.error('Failed to parse RSA key on Admin Edit Profile page:', err);
    }
  }
  ...
  return (
    <div className="animate-fade-up max-w-2xl mx-auto space-y-6 pb-10">
      <div className="pb-4 border-b border-slate-200/70">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Edit Admin Profile</h1>
        <p className="text-sm text-slate-500 mt-0.5">
          Update administrator profile information. Images are encrypted client-side.
        </p>
      </div>

      <div className="bg-white border border-slate-200/70 shadow-sm rounded-xl p-6 sm:p-8">
        <EditAdminProfileForm initialData={initialData} />
      </div>
    </div>
  );
}
