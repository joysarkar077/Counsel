    ...
    <div className="space-y-6 mb-8">
      ...
      <div className="space-y-6">
        {exhibits.map((ex, i) => {
          ...
          return (
            <div key={i} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow group">
              <div className="flex justify-between items-start mb-4 border-b border-slate-100 pb-4">
                <div>
                  <div className="flex items-center gap-3 mb-1.5">
                    <h4 className="font-bold text-slate-900 text-lg">{ex.number || 'Unnumbered Exhibit'}</h4>
                  ...
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">
                    Date: <span className="text-slate-700">{ex.date ? new Date(ex.date).toLocaleDateString() : 'Not set'}</span>
                  </p>
                </div>
                {!readOnly && (
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(i)} className="p-2 text-slate-500 hover:text-navy-core hover:bg-slate-100 rounded-lg transition-colors tooltip-trigger" title="Edit Exhibit">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                      </svg>
                    </button>
                    <button onClick={() => handleDelete(i)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors tooltip-trigger" title="Delete Exhibit">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
              ...
              <div className="mb-5">
                <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{ex.description || <span className="text-slate-400 italic">No description provided.</span>}</p>
              </div>
              {ex.fileUrl && (
                <div className="inline-flex flex-col bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 min-w-[250px]">
                  <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-200">
                    <svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                    </svg>
                    <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Attached File</span>
                  </div>
