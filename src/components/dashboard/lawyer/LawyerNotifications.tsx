...
import type { ILawyerNotification, NotificationCategory } from '@/types/lawyer-dashboard';

export interface LawyerNotificationsProps {
  readonly initialNotifications: readonly ILawyerNotification[];
}

export function LawyerNotifications({ initialNotifications }: LawyerNotificationsProps): React.ReactNode {
  const [notifications, setNotifications] = useState<readonly ILawyerNotification[]>(initialNotifications);
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter((n) => {
      ...
      if (activeTab === 'ASSIGNMENT') return n.category === 'assignment';
      return true;
    });
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm p-5 space-y-4">
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-1 border-b border-slate-100 pb-2 text-xs font-medium text-slate-500 overflow-x-auto">
        <button
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded-md transition-colors ${
            activeTab === 'ASSIGNMENT' ? 'bg-slate-900 text-white font-semibold' : 'hover:bg-slate-100 text-slate-600'
          }`}
        >
          Assignments
        </button>
      ...
      {filteredNotifications.length === 0 ? (
        <div className="py-6 text-center text-xs text-slate-400 rounded-lg border border-dashed border-slate-200">
              ...
              <div className="flex-1 min-w-0 space-y-0.5">
                ...
                {item.actionUrl && (
                  <div className="pt-1">
                    <Link
                      href={item.actionUrl}
                      className="text-[11px] font-semibold text-blue-600 hover:text-blue-800 transition-colors inline-flex items-center gap-1"
                    >
                      View →
                    </Link>
                  </div>
